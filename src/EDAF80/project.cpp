...
edaf80::project::run()
{
	...
	std::string computeShaderSource = readFile("F:/desktop/CourseFile/ComputerGraphics/src/EDAF80/computeShader3.glsl");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (!shader_reload_failed) {
			...
			glUniform1f(glGetUniformLocation(computeProgram, "numParticles"), spawner.particleCount);
